...
type SignIntakeCorrespondenceUploadBody = {
  fileName?: unknown;
  contentType?: unknown;
  fileSize?: unknown;
};
...
export async function POST(request: Request) {
  ...
  let body: SignIntakeCorrespondenceUploadBody;
  try {
  ...
  const storagePath = `${activeTenant.tenantId}/enquiries/intake/${Date.now()}-${safeId}-${originalFileName}${extension}`;
