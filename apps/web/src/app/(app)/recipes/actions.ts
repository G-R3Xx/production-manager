...
export async function createLabourRateAction(formData: FormData) {
  ...
  if (!name) redirect('/products?error=Labour%20name%20is%20required');
  ...
  redirect('/products?message=Labour%20rate%20created');
}
...
export async function createRecipeAction(formData: FormData) {
  ...
  const name = readString(formData, 'name');
  if (!productId || !name) redirect('/products?error=Product%20and%20recipe%20name%20are%20required');
  ...
  redirect(`/products?message=Component%20set%20created&productId=${productId}`);
}
...
export async function addRecipeComponentAction(formData: FormData) {
  ...
  const name = readString(formData, 'name');
  if (!recipeId || !name) redirect('/products?error=Recipe%20and%20component%20name%20are%20required');
  ...
  redirect('/products?message=Recipe%20component%20added');
}
