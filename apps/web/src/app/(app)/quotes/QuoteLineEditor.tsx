...
function savedLineMarkupMultiplier(configurationSnapshot: unknown, fallback: string | number | null | undefined): string | number {
  const snapshot = recordValue(configurationSnapshot);
  const pricing = recordValue(snapshot?.pricingSnapshot);
  const saved = Number(pricing?.markupMultiplier);
  return Number.isFinite(saved) && saved > 0 ? saved : String(fallback ?? "1.5");
}
...
export function QuoteLineEditor({ quoteId, line, product, materials, myobMatrixItems = [], pricingSettings, canOverrideMarkup = false, costingResources }: QuoteLineEditorProps) {
  ...
  const linePricingSettings = useMemo<FlowPricingSettings>(() => ({
    ...pricingSettings,
      ...
      <div style={{ border: "1px solid #bfdbfe", borderRadius: 18, padding: 14, background: "#fff", display: "grid", gap: 12 }}>
        {quickSnapshot.reconstructed ? (
          <section style={{ border: "1px solid #fed7aa", borderRadius: 14, padding: 11, background: "#fffbeb", display: "grid", gap: 4 }}>
            <strong style={{ color: "#9a3412" }}>Older quote line recovered in the current editor</strong>
            <span style={{ color: "#475467", fontSize: 13 }}>Known values were reconstructed from its saved summary. Saving this one-page form converts it to the current structured format.</span>
          </section>
        ) : null}
          ...
          {product ? (
            <fieldset style={{ border: "1px solid #dfe7f2", borderRadius: 14, padding: 12, background: "#fff", display: "grid", gap: 9 }}>
              ...
              <label style={{ display: "flex", gap: 9, alignItems: "start", fontWeight: 850 }}>
                ...
                <span><strong>Keep the product Production Method</strong><br /><small style={{ color: "#64748b" }}>Copies the linked materials, labour, charges and available option lists. These selections become its defaults.</small></span>
              </label>
              <label style={{ display: "flex", gap: 9, alignItems: "start", fontWeight: 850 }}>
