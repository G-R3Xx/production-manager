...
export default async function PurchasingPage({searchParams}:Props){
  ...
  const emailConfigured=Boolean(process.env.RESEND_API_KEY?.trim()&&(process.env.PURCHASE_ORDER_FROM_EMAIL?.trim()||company?.email?.trim()));
  ...
  return <div style={{maxWidth:1450,margin:"0 auto",display:"grid",gap:16}}>
    ...
    {!emailConfigured?<section style={{...card,borderColor:"#fdba74",background:"#fff7ed",color:"#9a3412"}}><b>Supplier email sending needs one deployment setting.</b> Add <code>RESEND_API_KEY</code> and a verified <code>PURCHASE_ORDER_FROM_EMAIL</code> (or use the Company email as the verified sender). MYOB sync remains independent if email is not configured.</section>:null}
