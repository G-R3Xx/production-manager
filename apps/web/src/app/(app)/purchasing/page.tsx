...
import {
  ...
  listPurchaseOrders,
  listPurchasableMaterialsForSupplier,
...
export default async function PurchasingPage({searchParams}:Props){
  ...
  const setupRequested=param(params,"setup")==="1";
  const historyRequested=param(params,"history")==="1";

  // Keep the normal Purchasing request entirely inside PM/Postgres. Older builds
  // loaded the full material records plus two complete MYOB reference collections
  // on every visit, which made this one of the slowest pages in the app.
  const [suppliers,orders,defaults]=await Promise.all([
    listPurchasingSuppliers(tenant.tenantId),
    listPurchaseOrders(tenant.tenantId),
    getPurchasingDefaults(tenant.tenantId)
  ]);
  const selectedId=requested||orders[0]?.id||"";
  const selected=orders.find((row)=>row.id===selectedId)??await getPurchaseOrder(tenant.tenantId,selectedId);
  const [lines,allowedMaterials]=selected?await Promise.all([
    listPurchaseOrderLines(tenant.tenantId,selected.id),
    listPurchasableMaterialsForSupplier(tenant.tenantId,selected.supplierId)
  ]):[[],[]];
  const [events,documents]=selected&&historyRequested?await Promise.all([
    listPurchaseOrderEvents(tenant.tenantId,selected.id),
    listPurchaseOrderDocuments(tenant.tenantId,selected.id)
  ]):[[],[]];
  ...
  // MYOB account/tax reference data is remote and can take seconds. Only fetch it
  // when a manager explicitly opens the setup editor; it must never block PO work.
  let connection:Awaited<ReturnType<typeof getMyobConnectionByTenantId>>=null;
  let tokenRecord:Awaited<ReturnType<typeof getMyobOauthTokenByTenantId>>=null;
  ...
  if(setupRequested){
    [connection,tokenRecord]=await Promise.all([
    ...
    <div style={{display:"grid",gridTemplateColumns:"340px minmax(0,1fr)",gap:16,alignItems:"start"}}>
      ...
      <main style={{display:"grid",gap:14}}>
        {!selected?<section style={card}><h2 style={{marginTop:0}}>Start your first PO</h2><p style={{color:"#667085"}}>Choose a supplier on the left, or use <b>New PO</b> from a Material or Supplier record.</p></section>:<>
          ...
          <section style={{...card,display:"grid",gap:10}}><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}><div><p style={{margin:0,fontSize:12,fontWeight:900,color:"#4f46e5"}}>PO HISTORY</p><h2 style={{margin:"6px 0"}}>Send + sync history</h2></div><Link href={`/purchasing?selected=${selected.id}${historyRequested?"":"&history=1"}`} style={{...secondary,display:"inline-flex",alignItems:"center",textDecoration:"none"}}>{historyRequested?"Hide history":"Load history"}</Link></div>{historyRequested?(documents.length||events.length?<>{documents.length?<div style={{display:"grid",gap:7,padding:"10px 0"}}><b style={{fontSize:13}}>Sent PDF copies</b>{documents.slice(0,5).map(document=><div key={document.id} style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",fontSize:12}}><span>{document.fileName} · {document.recipientEmail||"supplier"} · {eventTime(document.createdAt)}</span><a href={`/api/purchasing/purchase-orders/${selected.id}/pdf?documentId=${document.id}`} style={{color:"#1d4ed8",fontWeight:800}}>Download sent PDF</a></div>)}</div>:null}{events.slice(0,10).map(event=><div key={event.id} style={{display:"grid",gridTemplateColumns:"150px 1fr auto",gap:12,alignItems:"start",padding:"9px 0",borderTop:"1px solid #edf2f7"}}><b style={{fontSize:12,textTransform:"uppercase"}}>{event.eventType.replace(/_/g," ")}</b><span style={{fontSize:13,color:"#475467"}}>{event.message||"-"}</span><span style={{fontSize:11,color:"#98a2b3"}}>{eventTime(event.createdAt)}</span></div>)}</>:<span style={{color:"#667085",fontSize:13}}>No send or sync history yet.</span>):<span style={{color:"#667085",fontSize:13}}>History loads only when you need it, so it does not slow normal PO work.</span>}</section>
        </>}
        <section style={{...card,display:"grid",gap:12}}><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"center",flexWrap:"wrap"}}><div><p style={{margin:0,fontSize:12,fontWeight:900,color:"#4f46e5"}}>MYOB PURCHASING SETUP</p><h2 style={{margin:"6px 0"}}>Default purchase account + tax code</h2><p style={{margin:0,color:"#667085"}}>These are used when PM creates a purchased material as a new MYOB item. Remote MYOB lists load only when you open setup.</p></div>{!setupRequested?<Link href={`/purchasing${selected?`?selected=${selected.id}&setup=1`:"?setup=1"}`} style={{...secondary,display:"inline-flex",alignItems:"center",textDecoration:"none"}}>Change MYOB setup</Link>:<Link href={`/purchasing${selected?`?selected=${selected.id}`:""}`} style={{...secondary,display:"inline-flex",alignItems:"center",textDecoration:"none"}}>Close setup</Link>}</div>{!setupRequested?<div style={{display:"flex",gap:12,flexWrap:"wrap",fontSize:13,color:"#475467"}}><span><b>Expense account:</b> {defaults.expenseAccountDisplayId||defaults.expenseAccountName||"Not configured"}</span><span><b>Tax code:</b> {defaults.taxCode||"Not configured"}</span></div>:connection?.status!=="connected"?<div style={{padding:12,borderRadius:12,background:"#fff7ed",color:"#9a3412"}}>Connect MYOB first to load accounts and tax codes.</div>:<form action={savePurchasingDefaultsAction} style={{display:"grid",gridTemplateColumns:"1fr 1fr auto",gap:10,alignItems:"end"}}><label style={{display:"grid",gap:5,fontWeight:800,fontSize:13}}>Expense / Cost of Sales account<select name="expenseAccountUid" defaultValue={defaults.expenseAccountUid??""} style={input}><option value="">Choose MYOB account…</option>{refs.accounts.map(a=><option key={a.uid} value={a.uid}>{a.displayId} · {a.name} ({a.classification})</option>)}</select></label><label style={{display:"grid",gap:5,fontWeight:800,fontSize:13}}>Purchase tax code<select name="taxCodeUid" defaultValue={defaults.taxCodeUid??""} style={input}><option value="">Choose tax code…</option>{refs.taxCodes.map(t=><option key={t.uid} value={t.uid}>{t.code}{t.description?` · ${t.description}`:""}</option>)}</select></label><button style={button}>Save defaults</button></form>}</section>
      </main>
    </div>
  </div>;
}
