...
function selectedProductUrl(productId: string, query: string): string {
  ...
  return `/products/advanced?selected=${productId}${q}`;
}

function editProductUrl(productId: string, query: string, id: string): string {
  const q = query ? `&q=${encodeURIComponent(query)}` : "";
  return `/products/advanced?selected=${productId}${q}&editOption=${id}`;
}
        ...
        <section style={{ ...panelStyle, background: "#fff", gap: 12 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", justifyContent: "flex-end" }}>
              <a href="/products/advanced" style={{ color: filter === "deleted" ? "#64748b" : "#2563eb", fontWeight: 900, textDecoration: "none" }}>Active</a>
              <a href="/products/advanced?filter=deleted" style={{ color: filter === "deleted" ? "#2563eb" : "#64748b", fontWeight: 900, textDecoration: "none" }}>Deleted ({deletedProductCount})</a>
...
function productBuilderUrl(productId: string, query: string, part?: string): string {
  ...
  return `/products/advanced?${params.toString()}`;
}
