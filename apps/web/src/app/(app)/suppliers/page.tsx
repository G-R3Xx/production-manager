...
export default async function SuppliersPage({searchParams}:Props){
 ...
 const hasPendingMyobSync=suppliers.some(s=>["pending","syncing"].includes(String(s.payloadJson?.myobSyncStatus??"")));
  ...
  <div style={{display:"grid",gridTemplateColumns:"380px 1fr",gap:16,alignItems:"start"}}>
   ...
   <section style={{...panel,display:"grid",gap:16}}><form method="get" style={{display:"grid",gridTemplateColumns:"1fr auto",gap:12}}><input name="q" defaultValue={q} placeholder="Search suppliers" style={input}/><button style={ghost}>Search</button></form><div style={{display:"grid",gap:12}}>
    {suppliers.length===0?<div style={{color:"#475467"}}>No suppliers found.</div>:suppliers.map(s=><article key={s.id} style={{border:"1px solid #e5e7eb",borderRadius:16,padding:16,display:"grid",gap:10}}>
      <form action={updateSupplierAction} style={{display:"grid",gap:10}}><input type="hidden" name="supplierId" value={s.id}/><input name="displayName" defaultValue={s.displayName} style={input}/><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><input name="contactName" defaultValue={s.contactName??""} placeholder="Contact name" style={input}/><input name="phone" defaultValue={s.phone??""} placeholder="Phone" style={input}/></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><input type="email" name="email" defaultValue={s.email??""} placeholder="General email" style={input}/><input type="email" name="purchaseOrderEmail" defaultValue={s.purchaseOrderEmail??s.email??""} placeholder="Purchase order email" style={input}/></div><details style={{border:"1px solid #e5e7eb",borderRadius:12,padding:"10px 12px"}}><summary style={{cursor:"pointer",fontWeight:800,fontSize:13}}>Address</summary><div style={{marginTop:10}}><SupplierAddressFields payload={s.payloadJson}/></div></details><textarea name="notes" defaultValue={s.notes??""} rows={3} style={{borderRadius:10,border:"1px solid #d0d5dd",padding:12}}/><div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",flexWrap:"wrap"}}><div style={{display:"grid",gap:5}}><MyobSyncStatus status={readMyobSyncStatus(s.payloadJson?.myobSyncStatus,Boolean(s.myobUid))} linked={Boolean(s.myobUid)} error={s.payloadJson?.myobSyncError}/>{String(s.payloadJson?.myobSyncStatus??"")==="error"&&s.payloadJson?.myobSyncError?<span style={{fontSize:11,color:"#b42318",maxWidth:520}}>{String(s.payloadJson.myobSyncError)}</span>:null}</div><button style={dark}>Save</button></div></form>
