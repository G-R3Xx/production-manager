...
const input=(f:FormData,tenantId:string)=>({tenantId,name:s(f,"name"),machineType:s(f,"machineType")||"other",maxWidthMm:n(f,"maxWidthMm",""),speedValue:n(f,"speedValue"),speedUom:s(f,"speedUom")||"sqm_per_hour",hourlyCost:n(f,"hourlyCost"),setupMinutes:n(f,"setupMinutes"),inkCostPerSqm:n(f,"inkCostPerSqm"),colourImpressionCost:n(f,"colourImpressionCost"),monoImpressionCost:n(f,"monoImpressionCost"),processIds:f.getAll("processIds").map(String).filter(Boolean)});
