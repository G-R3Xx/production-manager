    ...
    <div style={{ maxWidth: 1240, margin: '0 auto', display: 'grid', gap: 20 }}>
      ...
      <section style={cardStyle}>
        ...
        <p style={{ margin: 0, color: '#475467', lineHeight: 1.6 }}>
          Suppliers sell you materials, not finished sellable products. Use this layer to define the raw sheets, rolls, stock and consumables that recipes will consume.
        </p>
      </section>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, minmax(0, 1fr))', gap: 16 }}>
        ...
        <div style={cardStyle}><div style={{ fontSize: 12, fontWeight: 700, color: '#4f46e5', textTransform: 'uppercase' }}>Model</div><div style={{ marginTop: 10, fontSize: 18, fontWeight: 700 }}>Supplier → Material → Recipe → Product</div><div style={{ marginTop: 8, color: '#475467' }}>Allocate stock from materials when products are sold.</div></div>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(340px, 420px) 1fr', gap: 20, alignItems: 'start' }}>
        <form action={createMaterialAction} style={{ ...cardStyle, display: 'grid', gap: 12 }}>
        ...
        <section style={{ ...cardStyle, display: 'grid', gap: 14 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 12 }}>
            <div>
              ...
              <p style={{ margin: '6px 0 0', color: '#475467' }}>These are the raw inputs that recipes should consume when a product is quoted or sold.</p>
            </div>
            <div style={{ color: '#667085', fontSize: 14 }}>{materials.length} total</div>
          </div>
          {materials.length === 0 ? (
            <div style={{ borderRadius: 16, border: '1px dashed #d0d5dd', padding: 24, color: '#475467' }}>No materials yet. Create sheet, roll, paper and hardware materials here before attaching them to recipes.</div>
          ) : (
