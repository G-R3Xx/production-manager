...
export default async function JobInvoicePage({ params, searchParams }: PageProps) {
  ...
  return <div style={{ maxWidth: 1500, margin: "0 auto", display: "grid", gap: 16 }}>
    ...
    <section style={card}>
      ...
      {unresolvedDraft ? <div style={{ border: "1px solid #fda29b", background: "#fff5f4", color: "#b42318", borderRadius: 14, padding: 14, fontWeight: 800 }}>Invoice {unresolvedDraft.invoiceNumber} has not been confirmed in MYOB. Use <strong>Recover / retry MYOB</strong> in Invoice history before creating another invoice for this job.</div> : <InvoiceBuilder
        jobId={job.id}
        quoteId={quoteId}
