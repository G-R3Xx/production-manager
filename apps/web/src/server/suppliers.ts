...
export async function listSuppliersForTenant(tenantId: string, search = ""): Promise<SupplierRecord[]> {
  ...
  const result = await pool.query<Omit<SupplierRecord, "payloadJson"> & { payloadJson: unknown }>(`
    SELECT id, tenant_id as "tenantId", myob_uid as "myobUid", display_name as "displayName",
      contact_name as "contactName", email, phone, is_active as "isActive", notes,
...
export async function upsertImportedSupplier(tenantId: string, input: {
  myobUid: string; displayName: string; contactName?: string | null; email?: string | null; phone?: string | null;
  isActive?: boolean; notes?: string | null; payloadJson?: Record<string, unknown>;
}): Promise<{ id: string }> {
  const result = await pool.query<{ id: string }>(`
    INSERT INTO app.suppliers (tenant_id,myob_uid,display_name,contact_name,email,phone,is_active,notes,payload_json,created_at,updated_at)
    VALUES ($1::uuid,$2::varchar,$3::varchar,$4::varchar,$5::varchar,$6::varchar,$7::boolean,$8::text,$9::jsonb,now(),now())
    ...
    DO UPDATE SET display_name=EXCLUDED.display_name, contact_name=EXCLUDED.contact_name, email=EXCLUDED.email,
      phone=EXCLUDED.phone, is_active=EXCLUDED.is_active, notes=COALESCE(EXCLUDED.notes,app.suppliers.notes),
      payload_json=COALESCE(app.suppliers.payload_json,'{}'::jsonb)||EXCLUDED.payload_json, updated_at=now()
    RETURNING id
  `, [tenantId,input.myobUid,input.displayName,input.contactName??null,input.email??null,input.phone??null,input.isActive??true,
